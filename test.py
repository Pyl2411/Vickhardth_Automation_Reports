...
class ColumnMappingDialog:
    ...
    def apply_mapping(self):
        ...
        if not self.mapping:
            response = messagebox.askyesno("No Mapping", 
                                         "No columns have been mapped. Apply empty mapping?")
            if not response:
                return
        
        self.result = self.mapping.copy()
        self.dialog.destroy()
...
class DataFetcherApp:
    ...
    def setup_export_tab(self):
        ...
        ttk.Button(mapping_btn_frame, text="🧪 Test Mapping", 
                  command=self.test_mapping, width=15).pack(side=tk.LEFT, padx=(10, 0))
...
"""
    ...
    def configure_column_mapping(self):
        ...
        if not raw_columns:
            # Fall back to the columns from current data
            raw_columns = self.current_data['columns']
        else:
            # Make sure we have all columns from current data
            data_columns = set(self.current_data['columns'])
            raw_columns_set = set(raw_columns)
            # Add any missing columns from current data
            missing_columns = data_columns - raw_columns_set
            if missing_columns:
                raw_columns.extend(list(missing_columns))
        ...
        if mapping:
            ...
            messagebox.showinfo("Mapping Applied", 
                              f"Column mapping applied successfully!\n\n"
                              f"Total fields mapped: {len(self.column_mapping)}\n\n"
                              f"You can now export to VPI Job Card.")
        else:
            self.log_message("Column mapping was cancelled", 'info')
    ...
    def test_mapping(self):
        ...
        if not self.column_mapping:
            messagebox.showinfo("No Mapping", "No column mapping configured yet")
            return
        
        # Create a simple test to verify mapping
        test_data = {
            'total_fields': len(self.column_mapping),
            'header_fields': 0,
            'table_fields': 0,
            'mapped_columns': list(self.column_mapping.keys()),
            'database_columns': list(self.column_mapping.values())
        }
        
        # Check field types
        excel_fields = {field['name']: field for field in self.exporter.get_excel_fields()}
        for excel_field in self.column_mapping.keys():
            field_info = excel_fields.get(excel_field, {})
            if field_info.get('is_table_column', False):
                test_data['table_fields'] += 1
            else:
                test_data['header_fields'] += 1
        
        # Show test results
        dialog = tk.Toplevel(self.root)
        dialog.title("Mapping Test Results")
        ...
        # Results
        ttk.Label(dialog, text="🧪 Mapping Test Results", 
                 font=('Segoe UI', 14, 'bold')).pack(pady=(20, 10))
        
        results_text = f"""
        ✅ Mapping Configuration Test
        
        Total Fields Mapped: {test_data['total_fields']}
        Header Fields: {test_data['header_fields']}
        Table Columns: {test_data['table_fields']}
        ...
        for excel_field, db_column in self.column_mapping.items():
            if count < 5:  # Show first 5
                results_text += f"\n    • {excel_field} → {db_column}"
                count += 1
            else:
                results_text += f"\n    ... and {test_data['total_fields'] - 5} more"
                break
    ...
    def reset_application(self):
        ...
        if messagebox.askyesno("Reset Application", 
            ...
            # Update status
            self.status_bar.config(text="Application reset")
            self.progress_label.config(text="Select a table and columns first")
            
            self.log_message("Application reset to initial state", 'info')
